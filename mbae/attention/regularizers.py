...
class AttentionFrobeniusNorm(regularizers.Regularizer):
    ...
    def __call__(self, x: KTensor) -> KTensor:
        ...
        sparsity_term = self.sparsity * K.eye(self.r) if self.sparsity else None
        group_norms = frobenius_norm(
            self_sim - sparsity_term if self.sparsity else self_sim,
            axes=[1, 2]
        )
    ...
    def get_config(self):
        return {'r': self.r, 'sparsity': self.sparsity, 'epsilon': self.epsilon}
